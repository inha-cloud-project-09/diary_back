...
import com.diary.api.service.OneLineDiaryService;
...
public class OneLineDiaryController {
    ...
    public ResponseEntity<ApiResponse<OneLineDiary>> createOneLineDiary(
        ...
        return ResponseEntity.ok(oneLineDiaryService.createOneLineDiary(oneLineDiary, user));
    }
    ...
    public ResponseEntity<ApiResponse<OneLineDiary>> updateOneLineDiary(
            ...
            @Valid @RequestBody OneLineDiary oneLineDiary,
            @AuthenticationPrincipal User user) {
        return ResponseEntity.ok(oneLineDiaryService.updateOneLineDiary(id, oneLineDiary, user));
    }
    ...
    @Operation(summary = "사용자 한 줄 일기 목록 조회", description = "특정 사용자의 한 줄 일기 목록을 조회합니다.")
    @GetMapping("/user")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiResponse<Page<OneLineDiary>>> getUserOneLineDiaries(
            @AuthenticationPrincipal User user,
    ...
    public ResponseEntity<ApiResponse<List<OneLineDiary>>> getOneLineDiariesByEmotion(
        ...
        return ResponseEntity.ok(oneLineDiaryService.getOneLineDiariesByEmotion(emotion));
    }

    @Operation(summary = "태그별 한 줄 일기 조회", description = "특정 태그의 한 줄 일기를 조회합니다.")
    @GetMapping("/tags")
    public ResponseEntity<ApiResponse<List<OneLineDiary>>> getOneLineDiariesByTags(
