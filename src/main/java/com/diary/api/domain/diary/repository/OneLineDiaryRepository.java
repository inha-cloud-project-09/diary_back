...
public interface OneLineDiaryRepository extends JpaRepository<OneLineDiary, Long> {
    ...
    // 태그별 공개 일기 조회
    @Query(value = "SELECT * FROM one_line_diary d WHERE d.is_public = true AND JSON_CONTAINS(d.tags, :tag)", nativeQuery = true)
    List<OneLineDiary> findByTagsContainingAndIsPublicTrue(@Param("tag") String tag);
    
    // 여러 태그로 공개 일기 조회
    @Query(value = "SELECT * FROM one_line_diary d WHERE d.is_public = true AND d.tags IN :tags", nativeQuery = true)
